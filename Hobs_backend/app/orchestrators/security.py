...
def create_staff_access_token(staff_id: str, client_id: str, merchant_id: str) -> str:
    """
    Staff-scoped JWT for HotelStaff dashboard login (separate from the
    merchant/owner login token above). Needed specifically so
    permission-change and audit-review actions know WHICH staff member is
    acting, not just which hotel account owns the session — see
    docs/PERMISSIONS.md. Deliberately its own token `type` so
    decode_access_token() can distinguish it from a merchant or client token.
    """
    expire = datetime.utcnow() + timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)
    to_encode = {
        "exp":         expire,
...
def decode_access_token(token: str) -> str | dict | None:
    ...
    Returns:
        ...
        - dict  → client token shape: { sub, merchant_id, type } OR
                  staff token shape: { sub, client_id, merchant_id, type }
